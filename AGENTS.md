...
- **Hover / tooltip.** Window `pointermove` + screen-space **ellipse** hit test on each bubble’s painted box (3D tilt foreshortens circles); sticky hover keeps the current bubble during peel. Name **Tooltip** above the bubble; **Popover** 8px below (12px placeholder copy, `border-rule`, paper shadow) — both portaled to `document.body`, anchored via RAF to `getBoundingClientRect()`.
...
- `Popover` — Card below an anchor (`anchorBottom` + `translate(-50%, gap)`). `border-rule`, `bg-surface`, `text-xs`, subtle `paper-shadow`. `pointer-events: none`; used with people-cloud hover placeholder copy.
