...
- **Chrome stripped on mobile.** `page.tsx`: gutter border and four dotted rules are `hidden md:block`. `LeftPageText` is `hidden md:flex`. **Ryan P.** and social links are `hidden` below `md` (desktop frame insets only). Book + button row are vertically centered as one stack in `dvh` (`MOBILE_BOOK_TOP` / `MOBILE_BUTTON_ROW_TOP` in `constants.ts`; tokens `--mobile-stack-gap`, `--mobile-button-row-height`).
