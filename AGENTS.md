...
- **Chrome stripped on mobile.** `page.tsx`: gutter border and four dotted rules are `hidden md:block`. `LeftPageText` is `hidden md:flex`. **Ryan P.** and social links sit in one row **above** the book (`top: calc(50vh - var(--book-height) / 2 - 48px)` — same 48px gap as the button row below). Desktop keeps frame insets (`bottom: 44px`, `left/right: 52px`).
